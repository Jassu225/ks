...
- **`${CLAUDE_PLUGIN_DATA}`** (`~/.claude/plugins/data/ks-flow-<marketplace>/`) is the persistent state dir that survives updates → home for the SQLite DB, `events.jsonl`, and installed `node_modules`. **`${CLAUDE_PLUGIN_ROOT}`** is ephemeral (recycled ~7 days post-update) → only for referencing bundled scripts, never for state. Both are exported as env vars to hook processes.
