...
if ! throttled "$HOOK_SID"; then
  ...
  case "$HOOK_TOOL" in
    ExitPlanMode) notify "$HOOK_SID" "Plan ready for your review" "$NOTIFY_DETAIL" "$NOTIFY_LABEL" ;;
    *) notify "$HOOK_SID" "Claude is asking you a question" "$NOTIFY_DETAIL" "$NOTIFY_LABEL" ;;
  esac
fi
exit 0
