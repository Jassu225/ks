...
**Windowed timestamps are clip-relative.** With `--from 2388 --to 2853`, the manifest and `frames.json` start at `00:00:00.000`, not `00:39:48`. Add the `--from` offset before quoting a time to a human or feeding it back to `--from/--to`, and say which base you're using. Verified: that window's last frame reads `00:07:30.000`, i.e. 2838s in source time. A full-file run needs no offset.
